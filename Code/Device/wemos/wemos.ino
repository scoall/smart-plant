...
void loop() {
  ...
  if(cell.available() > 0)
  {
    ...
    if (incoming_char =='M'){
      ...
      moisture=true;
      humidity, light, temp = false;
    }
    else if (incoming_char =='H'){
      Serial.print("Humidity");
      humidity=true;
      moisture, light, temp = false;
    }
    else if (incoming_char =='T'){
      Serial.print("Temp");
      temp=true;
      moisture, light, humidity = false;
    }
    else if (incoming_char =='L'){
      Serial.print("Light");
      light=true;
      moisture, humidity, temp = false;
    }
    else{
      char value = incoming_char;
      Serial.print(value);
        
      /*if (moisture==true){Serial.println("M");}
      else if (humidity==true){Serial.println("H");}
      else if (light==true){Serial.println("L");}
      else if (temp==true){Serial.println("T");}
      /*Serial.print("M is: ");
      Serial.print(m);
      Serial.print("H is: ");
      Serial.print(h);
